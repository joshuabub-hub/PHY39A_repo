...
// Only one H-bridge pin is ever driven at a time. Which pin is "active" is
// read from the direction switch each loop instead of being fixed in code.
int activePwmPin = hBridgeCoolPwmPin;
int inactivePwmPin = hBridgeHeatPwmPin;
...
void updateActivePwmPin() {
  // directionSwitchPin uses INPUT_PULLUP: open switch reads HIGH (cooling),
  // switch closed to GND reads LOW (heating).
  if (digitalRead(directionSwitchPin) == HIGH) {
    activePwmPin = hBridgeCoolPwmPin;
    inactivePwmPin = hBridgeHeatPwmPin;
  } else {
    activePwmPin = hBridgeHeatPwmPin;
    inactivePwmPin = hBridgeCoolPwmPin;
  }
}
...
void printHumanReadable(float temperatureC, int pwm) {
  int heatCool = (activePwmPin == hBridgeHeatPwmPin) ? 1 : 0;
